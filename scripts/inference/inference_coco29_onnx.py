"""Video inference script for COCO29 RTMPose model using ONNX with pre-computed bounding boxes.

This script loads pre-computed person bounding boxes from a .npy file and runs
pose estimation using an ONNX model on each frame of a video, then saves the output
with visualized keypoints and bounding boxes.
"""
...
def main():
    # ======================== CONFIGURATION ========================
    # Paths
    onnx_model_path = '/home/marco/Desktop/phoenix/models/rtmPose-l_coco29/rtmpose_coco29.onnx'  # UPDATE THIS PATH
    ...
    output_path = 'output_video_onnx.mp4'

    # Settings
    input_size = (192, 256)  # (width, height) - must match ONNX model
    ...
    print("=" * 60)
    print("RTMPose COCO29 ONNX Video Inference")
    print("=" * 60)
    ...
    while cap.isOpened() and frame_idx < len(bboxes_data):
        ...
        if bbox_score > 0 and bbox[2] > bbox[0] and bbox[3] > bbox[1]:
            # Preprocess image using MMPose-compatible pipeline
            input_tensor, warp_mat = preprocess_image_mmpose(frame, bbox, input_size, padding=1.0)
